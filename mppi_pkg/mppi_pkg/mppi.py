...
class BasicMPPIDynamicHorizonPlanner(Node):
    ...
    def __init__(self):
        ...
        # Real-car LiDAR self-filter.
        # The physical car can see its own bumper/LiDAR mount/cables as a fake
        # obstacle around 0.15 m in front. If we do not remove those points,
        # the planner stays in collision=True / escape=True forever.
        self.min_real_lidar_range = 0.05
        self.front_self_filter_range = 0.32
        self.front_self_filter_angle_deg = 24.0
        self.self_filter_x = 0.34
        self.self_filter_y = 0.22
        ...
        self.max_speed = 3.95
        self.min_speed = 0.25
        self.lateral_accel_limit = 4.95

        self.max_accel_step = 0.44
        self.straight_accel_step = 0.84
        self.max_decel_step = 0.50
        self.emergency_decel_step = 0.98
        self.max_steer_step = 0.42

        self.min_planning_speed = 1.00
        self.max_planning_speed = 3.20
        ...
        self.turn_lookahead_min_time = 1.22
        ...
        # V5.3 braking/speed tune:
        # Keep the good V5.1 line, but make braking less abrupt and exits faster.
        # Bad fallback paths still get trimmed early, instead of waiting for escape.
        self.clean_exit_accel_step = 0.68
        self.corner_accel_step = 0.56
        self.fallback_bad_speed = 0.82
        self.fallback_soft_speed = 1.10
        self.fallback_medium_speed = 1.35

        # V5.4 corner-speed-carry tune:
        # V5.3 was stable but sometimes over-braked in the middle of corners.
        # These floors apply only when the selected path is safe enough.
        self.safe_corner_min_speed = 1.28
        self.clean_corner_min_speed = 1.52
        self.fallback_corner_min_speed = 1.05
        ...
        self.get_logger().info(
            'BASIC MPPI V5.3 started: V5.1 base + smoother braking + faster clean exits.'
        )
    ...
    def valid_range(self, scan, r):
        ...
        # Keep only physically meaningful LiDAR returns.
        # Very tiny ranges are almost always invalid/noisy on the real car.
        if r < self.min_real_lidar_range:
            return False
    ...
    def sector_values(self, scan, min_deg, max_deg):
        ...
        for i, r in enumerate(scan.ranges):
            ...
            # Real-car front self-filter: ignore very close returns directly
            # in front of the LiDAR. These are usually the car body or mount,
            # not a real wall. Side sectors are still allowed to see close walls.
            if (
                abs(angle_deg) <= self.front_self_filter_angle_deg
                and r < self.front_self_filter_range
            ):
                continue
    ...
    def scan_to_obstacle_points(self, scan):
        ...
        for i in range(0, len(scan.ranges), 2):
            ...
            # Ignore points inside the car's own nose/body footprint.
            # Without this, the rollout collision checker sees a permanent
            # obstacle at the start of every path and reports clearance=0.00.
            if 0.0 <= x <= self.self_filter_x and abs(y) <= self.self_filter_y:
                continue
    ...
    def choose_best_rollout(self, results, target_info):
        ...
        if fallback:
            return min(
                ...
                key=lambda i: (
                    results[i]['cost']
                    + 11.5 * abs(results[i]['lateral_offset'])
                    + 19.0 * results[i]['overshoot_score']
        ...
        best_i = max(
            ...
            key=lambda i: (
                results[i]['clearance']
                - 0.82 * abs(results[i]['lateral_offset'])
                - 1.00 * results[i]['wrong_way_score']
                - 1.15 * results[i]['overshoot_score']
    ...
    def choose_speed(self, steer, best, target_info, mode, horizon_mode):
        ...
        # Wall-side speed discipline. V5.1 was stable but sometimes over-braked
        # after exits. Keep the close-wall protection, but let the car carry
        # more speed when the side wall is not extremely close.
        if min_side < 0.34:
            target_speed = min(target_speed, 1.05)
        elif min_side < 0.43 and self.previous_speed > 2.00:
            target_speed = min(target_speed, 1.65)
        elif min_side < 0.52 and self.previous_speed > 2.55:
            target_speed = min(target_speed, 2.20)
        ...
        stable_turn_path = (
            turn_strength > 0.50
            and clearance > 0.27
            and lateral_error < 0.32
            and wrong_way_score < 0.05
            and overshoot_score < 0.16
            and not collision
        )
        ...
        safe_corner_carry = (
            ...
            and wrong_way_score < 0.05
            and overshoot_score < 0.18
            and front_min > 0.72
            and not collision
            and mode != 'MAX_CLEARANCE'
        )
        ...
        clean_corner_carry = (
            ...
            and overshoot_score < 0.10
            and mode in ['PREFERRED', 'SAFE']
        )

        # Earlier but smoother braking: brake before the tight part, then allow
        # more speed if the selected turn path is clean. This should feel less
        # like stop-go braking while still protecting the two problem corners.
        if front_min < self.pre_turn_front_near:
            # Brake before the tightest point, but do not crawl if the chosen
            # arc is clean and already inside the correct turn.
            target_speed = min(target_speed, 1.32 if stable_turn_path else 0.95)
        elif front_min < self.pre_turn_front_mid:
            target_speed = min(target_speed, 1.78 if stable_turn_path else 1.34)
        elif front_min < self.pre_turn_front_start and self.previous_speed > 1.85:
            target_speed = min(target_speed, 2.28 if stable_turn_path else 1.96)
        ...
        # V5.3: keep fallback selection unchanged, but improve braking.
        # If fallback is ugly, brake earlier and more clearly instead of entering
        # the wall then dropping into escape. If fallback is only slightly wide,
        # keep enough speed so the lap time does not suffer.
        if mode == 'FALLBACK_LINE':
            if overshoot_score > 0.34 or lateral_error > 0.48:
                target_speed = min(target_speed, self.fallback_bad_speed)
            elif overshoot_score > 0.26 or lateral_error > 0.36:
                target_speed = min(target_speed, self.fallback_soft_speed)
            elif overshoot_score > 0.20 or lateral_error > 0.28:
                target_speed = min(target_speed, self.fallback_medium_speed)

        # Carry more speed through corners only when the chosen rollout is not
        # ugly. This fixes the slow mid-corner feel without touching steering,
        # horizon, or fallback selection.
        if clean_corner_carry:
            target_speed = max(target_speed, min(curvature_speed, self.clean_corner_min_speed))
        elif safe_corner_carry:
            target_speed = max(target_speed, min(curvature_speed, self.safe_corner_min_speed))
        elif (
            mode == 'FALLBACK_LINE'
            and turn_strength > 0.55
            and clearance > 0.22
            ...
            target_speed = max(target_speed, min(curvature_speed, self.fallback_corner_min_speed))
        ...
        if horizon_mode == 'SHORT' and clearance < 0.24:
            target_speed = min(target_speed, 1.15)
        elif horizon_mode == 'SHORT' and clearance < 0.28 and not safe_corner_carry:
            target_speed = min(target_speed, 1.15)
        ...
        if target_speed > self.previous_speed:
            ...
            clean_exit = (
                horizon_mode in ['MID', 'LONG']
                and turn_strength < 0.32
                and abs_steer < 0.13
                and lateral_error < 0.28
                and clearance > 0.34
                and front > 1.80
            ...
            if straight_fast:
                accel_step = self.straight_accel_step
            elif clean_exit:
                accel_step = self.clean_exit_accel_step
            elif clean_corner:
                accel_step = self.corner_accel_step
            else:
                accel_step = self.max_accel_step

            accel_step *= self.clamp(1.0 - 0.08 * turn_strength, 0.82, 1.0)
            speed = min(target_speed, self.previous_speed + accel_step)
        else:
            # Smooth braking most of the time, stronger braking only when we are
            # really close to a wall/turn. This is meant to remove the harsh
            # stop-go feel without arriving late to the same tight corners.
            if emergency:
                decel_step = self.emergency_decel_step
            elif clean_corner_carry:
                # Keep the car flowing through the corner. Do not dump speed
                # in the middle when the rollout is clean.
                decel_step = 0.42
            elif safe_corner_carry:
                decel_step = 0.50
            elif (
                front_min < 1.05
                or clearance < 0.23
                or (mode == 'FALLBACK_LINE' and (overshoot_score > 0.26 or lateral_error > 0.34))
            ):
                decel_step = 0.72
            elif front_min < 1.65 and turn_strength > 0.55:
                decel_step = 0.56
            else:
    ...
    def scan_callback(self, scan):
        ...
        if self.callback_count % 8 == 0:
            self.get_logger().info(
                ...
                f'self_filter={self.front_self_filter_range:.2f}m, '
